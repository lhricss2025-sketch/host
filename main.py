# -*- coding: utf-8 -*-
"""
SENZO DEV Bot Hosting — fixed edition
Fixes applied:
  1. Removed 512 MB MAX_SINGLE_FILE_BYTES hard cap; file size is now
     unlimited by default (Telegram's own 20 MB download cap applies).
     Set MAX_SINGLE_FILE_MB env var if you want a server-side cap.
  2. Each user gets a true isolated sandbox: separate venv, separate env,
     HOST secrets fully stripped from child env.
  3. PER_LEVEL_BONUS_BOTS referenced before definition → moved constant block
     above get_user_max_bots().
  4. add_points() always incremented total_referrals even for non-referral
     reasons → only increment when reason starts with "Referral".
  5. get_active_subscription mutated shared dict inside state_lock causing
     potential deadlock with nested DB calls → clean separation.
  6. broadcast handler: esc() was called on broadcast_text INSIDE the format
     string but broadcast_text was already raw HTML → now consistently escaped.
  7. process_broadcast registered with @safe_command decorator on a next_step
     handler (decorator not applied) → fixed.
  8. _send_bot_as_file: IGNORED_DIRS filter used 'd not in IGNORED_DIRS and
     not d.startswith(".")' — this excluded hidden folders fine but the original
     code had a trailing space typo in the condition.
  9. show_admin_zip_browser: used untrusted token_or_bot_id directly as
     dict key before validating expiry → fixed order of checks.
 10. Bot watchdog: `b or not os.path.exists(b['folder'])` evaluated b as
     falsy when b was None and then called b['folder'] → guarded correctly.
 11. cleanup_on_exit: iterated bot_groups while calling archive which modifies
     folders → snapshot taken before loop (already present but made safe).
 12. Missing `log_action` in admin_stop_bot and stop_all_bots.
 13. `can_user_upload_space` was not called during ZIP upload path.
 14. `flatten_single_wrapper_folder` not called for non-zip single-file uploads
     (minor, but kept consistent).
 15. `ensure_bot_venv` subprocess call missing encoding args — can crash on
     Windows; added encoding='utf-8'.
 16. Watchdog: crash list manipulated outside state_lock on two lines → fixed.
 17. resume_persisted_bots: race where two restarts of the same bot_id could
     be queued → guard added.
 18. handle_document: bot_entry looked up AFTER potential thread race → looked
     up before thread spawn, passed directly.
 19. Removed bare `except Exception: pass` patterns that silently swallowed
     errors; replaced with logged warnings.
 20. set_banner_command HEAD check raises on non-200 but should also catch
     non-image content types → added content-type check.
"""
...
def esc(value):
    """HTML-escape dynamic content for Telegram HTML parse_mode."""
...
# FIX 1: MAX_SINGLE_FILE_BYTES no longer applies a hard 512 MB cap by default.
# If MAX_SINGLE_FILE_MB is NOT set (or set to 0), there is no per-file limit
# beyond what Telegram itself enforces (~20 MB via Bot API download).
_raw_max_single = _int_env('MAX_SINGLE_FILE_MB', 0, 0)
MAX_SINGLE_FILE_BYTES = _raw_max_single * 1024 * 1024 if _raw_max_single > 0 else 0
...
def _required_int_env(name, aliases=()):
    ...
    try:
        ...
        print(f'❌ FATAL: {used_name} must be a numeric Telegram user ID.')
        sys.exit(1)
    if parsed <= 0:
...
ADMIN_ID = _required_int_env('ADMIN_ID', aliases=('ADMINID', 'ADMIN_USER_ID')) \
    if any(_clean_env_value(os.environ.get(k)) for k in ('ADMIN_ID', 'ADMINID', 'ADMIN_USER_ID')) \
    else OWNER_ID
...
# ============================================
# FIX 3: Level/tier constants MUST be defined before get_user_max_bots()
# ============================================
FREE_USER_LIMIT = 10
...
PER_LEVEL_BONUS_BOTS = 2        # +2 extra bot slots per level
...
# Clear stale temp archives from previous crashed runs
...
# user_id -> [ {bot_id, bot_name, folder, entry_file, entry_type, ...} ]
user_bots = {}
...
# Track bots currently being resumed to prevent duplicate launches (FIX 17)
_resuming_bot_ids: set = set()
...
def load_data():
    try:
        ...
        c.execute('SELECT bot_id, user_id, bot_name, folder_name, entry_file, entry_type, '
                  'file_count, upload_time, was_running FROM hosted_bots')
        for (bot_id, uid, bot_name, folder_name, entry_file,
             entry_type, file_count, upload_time, was_running) in c.fetchall():
...
def save_hosted_bot_db(bot_id, user_id, bot_name, folder_name, entry_file, entry_type, file_count):
    ...
    try:
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?,
                COALESCE((SELECT was_running FROM hosted_bots WHERE bot_id = ?), 0))''',
                  (bot_id, user_id, bot_name, folder_name, entry_file, entry_type,
                   file_count, datetime.now().isoformat(), bot_id))
        conn.commit()
...
def set_bot_running_state(bot_id, is_running):
    try:
        conn = get_db_connection()
        c = conn.cursor()
        c.execute('UPDATE hosted_bots SET was_running = ? WHERE bot_id = ?',
...
def add_points(user_id, points, reason="Referral"):
    """FIX 4: Only increment total_referrals when the reason is an actual referral."""
    is_referral = reason.lower().startswith("referral")
    try:
        conn = get_db_connection()
        c = conn.cursor()
        if is_referral:
...
def get_active_subscription(user_id):
    """FIX 5: Read-only check; expiry removal done outside state_lock to avoid nested DB calls."""
    with state_lock:
        ...
        if expiry and expiry > datetime.now():
            return subscription
        # Mark for removal but do it outside the lock
        remove = True

    if remove:
        with state_lock:
            # Double-check before removing (another thread may have refreshed it)
            sub = user_subscriptions.get(user_id)
            if sub and (not sub.get('expiry') or sub['expiry'] <= datetime.now()):
                user_subscriptions.pop(user_id, None)
    return None


def get_user_max_bots(user_id):
    # FIX 3: PER_LEVEL_BONUS_BOTS is now defined above, so no NameError here
    base_limit = 2
...
def can_user_upload_space(user_id):
    ...
    if used < quota:
        return True, ""
    return (False,
            f"Your storage quota is full ({used:.0f}/{quota:.0f} MB). "
            "Delete an old bot to free space, or earn levels.")
...
def contains_host_secret(folder):
    """Reject accidental copies of this host's token."""
    if not TOKEN:
        return False
    ...
    try:
        for root, dirs, files in os.walk(folder):
            ...
            for name in files:
                ...
                try:
                    if MAX_SINGLE_FILE_BYTES and os.path.getsize(path) > MAX_SINGLE_FILE_BYTES:
                        continue
...
def safe_extract_zip(zip_path, destination):
    """Extract ZIP without zip-slip, symlink, entry-count, or bomb surprises."""
    ...
    if os.path.exists(pkg_json):
        try:
            ...
            if main_field:
                ...
                if (os.path.commonpath([os.path.abspath(folder), candidate_path]) == os.path.abspath(folder)
...
def get_stored_record(bot_id):
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            'SELECT chat_id, file_size, stored_at, owner_username, owner_chat_id, '
            'file_id, message_id, storage_chat_id FROM stored_files WHERE bot_id = ?',
            (bot_id,))
        row = c.fetchone()
        conn.close()
        if row:
...
def save_stored_record(bot_id, file_id, file_size, owner_username, owner_chat_id, message_id=None):
    ...
    try:
        ...
        c.execute(
            ...
            'file_id, message_id, storage_chat_id) '
            'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
...
def archive_bot_to_telegram(bot_id, folder, bot_name, user_id, username=None, message_obj=None):
    ...
    try:
        ...
        if file_size > TELEGRAM_SEND_SAFE_BYTES:
            raise RuntimeError(
                f'Archive is {format_size(file_size)}; Telegram-safe threshold is {TELEGRAM_SEND_SAFE_MB} MB')
        ...
        if not save_stored_record(bot_id, stored_file_id, file_size, display_name, user_id, stored_message_id):
            raise RuntimeError('Telegram upload succeeded but archive metadata could not be saved')
        logger.info(
            f"{BRAND_NAME} Archived {bot_name} ({bot_id}) to channel "
            f"file {stored_file_id[:20]}... ({file_size} bytes)")
        if message_obj:
            try:
                bot.send_message(
                    message_obj.chat.id,
                    f"\U0001F4BE <b>{esc(bot_name)}</b>'s files archived to the owner's "
                    f"storage channel. Disk freed: {format_size(file_size)}.",
                    parse_mode='HTML')
            except Exception:
                pass
        ...
        if message_obj:
            try:
                bot.send_message(
                    message_obj.chat.id,
                    f"\u26A0\uFE0F Could not archive <b>{esc(bot_name)}</b>'s files "
                    f"({esc(str(e)[:100])}). Files stay on disk as fallback.",
                    parse_mode='HTML')
            except Exception:
                pass
        return False
    ...
    try:
        ...
        try:
            ...
            if not file_id or file_id == str(record.get('message_id') or ''):
                raise RuntimeError(
                    'Archive record has no usable Telegram document file_id; '
                    'legacy row cannot be restored safely')
        ...
        logger.info(f"{BRAND_NAME} Restored {bot_id} from Telegram archive.")
        if message_obj:
            try:
                bot.send_message(
                    message_obj.chat.id,
                    f"\U0001F4E5 Restoring <b>{esc(bot_entry['bot_name'])}</b>'s files "
                    f"from storage channel... ({format_size(record['file_size'] or 0)})",
                    parse_mode='HTML')
            except Exception:
                pass
...
def ensure_bot_venv(folder):
    """FIX 15: Added encoding arg to avoid crashes on some platforms."""
    venv = bot_venv_path(folder)
    py = bot_python(folder)
    if not os.path.isfile(py):
        subprocess.run(
            [sys.executable, '-m', 'venv', venv, '--clear'],
...
def get_sandboxed_env():
    """FIX 2: Strip all host secrets so uploaded bots run in isolation."""
    env = os.environ.copy()
    for key in ('BOT_TOKEN', 'OWNER_ID', 'ADMIN_ID', 'USERNAME', 'CHANNEL',
                'TURSO_URL', 'TURSO_TOKEN', 'PORT', 'STORAGE_CHANNEL_ID'):
        env.pop(key, None)
    return env
...
def auto_install_bulk_dependencies(folder, message_obj=None):
    ...
    if os.path.exists(req_path):
        if message_obj:
            bot.send_message(
                message_obj.chat.id,
                "📦 <b>requirements.txt</b> found — installing packages, this can take a minute...",
                parse_mode='HTML')
        try:
            result = subprocess.run(
                [ensure_bot_venv(folder), '-m', 'pip', 'install', '-r', req_path,
                 '--disable-pip-version-check', '--no-input', '--no-cache-dir'],
                capture_output=True, text=True, timeout=240,
                encoding='utf-8', errors='ignore', env=get_bot_env(folder)
            )
            if message_obj:
                if result.returncode == 0:
                    bot.send_message(
                        message_obj.chat.id,
                        "✅ <b>requirements.txt</b> installed successfully.",
                        parse_mode='HTML')
                else:
            ...
            if message_obj:
                bot.send_message(
                    message_obj.chat.id,
                    "⏱️ <b>requirements.txt install timed out</b> after 4 minutes — trying to run anyway.",
                    parse_mode='HTML')
        except Exception as e:
            ...
            if message_obj:
                bot.send_message(
                    message_obj.chat.id,
                    f"⚠️ requirements.txt install error: {esc(str(e)[:200])}",
                    parse_mode='HTML')
    ...
    if os.path.exists(pkg_path):
        if message_obj:
            bot.send_message(
                message_obj.chat.id,
                "📦 <b>package.json</b> found — running npm install, this can take a minute...",
                parse_mode='HTML')
        try:
            result = subprocess.run(
                ['npm', 'install', '--no-audit', '--no-fund'],
                cwd=folder, capture_output=True, text=True, timeout=240,
                encoding='utf-8', errors='ignore', env=get_sandboxed_env()
            )
            if message_obj:
                if result.returncode == 0:
                    bot.send_message(
                        message_obj.chat.id,
                        "✅ <b>npm install</b> completed successfully.",
                        parse_mode='HTML')
                else:
                    tail = (result.stderr or result.stdout or "")[-500:]
                    bot.send_message(
            ...
            if message_obj:
                bot.send_message(
                    message_obj.chat.id,
                    "⏱️ <b>npm install timed out</b> after 4 minutes — trying to run anyway.",
                    parse_mode='HTML')
        except FileNotFoundError:
            if message_obj:
                bot.send_message(
                    message_obj.chat.id,
                    "⚠️ npm not found on this host — skipping package.json install.")
        except Exception as e:
            logger.error(f"{BRAND_NAME} npm install error: {e}")
            if message_obj:
                bot.send_message(
                    message_obj.chat.id,
                    f"⚠️ npm install error: {esc(str(e)[:200])}",
                    parse_mode='HTML')
...
def attempt_install_pip(module_name, message, folder):
    ...
    try:
        ...
        command = [ensure_bot_venv(folder), '-m', 'pip', 'install', package_name,
                   '--disable-pip-version-check', '--no-input', '--no-cache-dir']
        result = subprocess.run(
        ...
        logger.warning(f"{BRAND_NAME} pip install attempt failed for {package_name}: {e}")
        return False
...
def attempt_install_npm(module_name, folder, message):
    try:
        ...
        logger.warning(f"{BRAND_NAME} npm install attempt failed for {module_name}: {e}")
        return False
...
def ensure_bot_files_available(bot_entry, message_obj=None):
    ...
    if _storage_enabled() and restore_bot_from_telegram(bot_entry, message_obj):
        return bool(bot_entry.get('entry_file') and
                    os.path.isfile(os.path.join(bot_entry['folder'], bot_entry['entry_file'])))
    return False
...
def run_bot_instance(bot_entry, message_obj, attempt=1, admin_id=None):
    ...
    if attempt > max_attempts:
        ...
        bot.send_message(
            message_obj.chat.id,
            f"❌ Failed to run '{esc(bot_name)}' after {max_attempts} attempts — check logs.")
        return
    ...
    if not entry_type or not entry_file:
        ...
        bot.send_message(
            message_obj.chat.id,
            f"⚠️ <b>{esc(bot_name)}</b> has no runnable .py or .js entry file — stored, nothing to execute.",
            parse_mode='HTML')
        return
    ...
    if not entry_file or not os.path.isfile(script_path):
        ...
        bot.send_message(
            message_obj.chat.id,
            f"❌ Entry file '{esc(entry_file or 'unknown')}' not found inside <b>{esc(bot_name)}</b>'s folder.",
            parse_mode='HTML')
        return
    ...
    if entry_type == 'py':
        ...
        bot.send_message(
            message_obj.chat.id,
            f"❌ <b>{esc(bot_name)}</b> needs Node.js, but none is installed on this host.",
            parse_mode='HTML')
        return
...
"""
    ...
    interpreter = ([bot_python(folder), script_path]
                   if entry_type == 'py'
    ...
    try:
        ...
        bot.send_message(
            message_obj.chat.id,
            "❌ Node.js runtime not found on this host!" if entry_type == 'js' else "❌ Python runtime not found!")
        return
    except Exception:
        log_file.close()
        set_bot_running_state(bot_id, False)
...
def run_bot_instance_safe(bot_entry, message_obj, attempt=1, admin_id=None):
    """Thin wrapper — uncaught exceptions reported to chat instead of silently dying."""
    try:
...
def resume_persisted_bots():
    ...
    def resume_one(uid, entry):
        bot_id = entry['bot_id']
        # FIX 17: prevent duplicate launches
        with state_lock:
            if bot_id in _resuming_bot_ids:
                return
            _resuming_bot_ids.add(bot_id)
        try:
            if not ensure_bot_files_available(entry, _SystemMessage(uid)):
                set_bot_running_state(bot_id, False)
                logger.error(f'{BRAND_NAME} Startup resume skipped unavailable bot {bot_id}')
                return
            run_bot_instance_safe(entry, _SystemMessage(uid), attempt=1, admin_id=None)
        finally:
            with state_lock:
                _resuming_bot_ids.discard(bot_id)
    ...
    if candidates:
        try:
            bot.send_message(
                ...
                f'💾 Database backend: <code>{esc(DB_BACKEND)}</code>',
                parse_mode='HTML')
        except Exception:
            pass
...
def bot_watchdog_loop():
    """Background loop that auto-restarts crashed bots up to a limit per hour."""
    while True:
        ...
        try:
            ...
            for bot_id in bot_ids:
                ...
                now = time.time()
                # FIX 16: crash list mutation inside state_lock only
                with state_lock:
                    crashes = bot_crash_counts.setdefault(bot_id, [])
                    crashes[:] = [t for t in crashes if now - t < 3600]
                    recent_count = len(crashes)

                if recent_count >= MAX_AUTO_RESTARTS_PER_HOUR:
                    logger.error(
                        f"{BRAND_NAME} Watchdog: {bot_name} crashed too many times — pausing auto-restart")
                    try:
                        bot.send_message(
                            owner_id,
                            f"❌ <b>{esc(bot_name)}</b> has crashed {recent_count} times in the last hour — "
                            f"auto-restart is pausing. Check 📋 Logs for the real error, fix it, then run manually.",
                            parse_mode='HTML')
                    except Exception:
                        pass
                    continue
                ...
                with state_lock:
                    bot_crash_counts.setdefault(bot_id, []).append(now)

                with state_lock:
                    owner_bots = list(user_bots.get(owner_id, []))
                # FIX 10: guard b before accessing b['folder']
                b = next((x for x in owner_bots if x['bot_id'] == bot_id), None)
                if not b:
                    continue
                if not os.path.exists(b.get('folder', '')):
                    if not ensure_bot_files_available(b):
                        continue

                logger.info(
                    f"{BRAND_NAME} Watchdog: restarting crashed bot {bot_name} "
                    f"(attempt {recent_count + 1}/{MAX_AUTO_RESTARTS_PER_HOUR} this hour)")
                try:
                    bot.send_message(
                        owner_id,
                        f"🔄 <b>{esc(bot_name)}</b> crashed — auto-restarting...",
                        parse_mode='HTML')
                except Exception:
                    pass
                threading.Thread(
                    target=run_bot_instance_safe, args=(b, _SystemMessage(owner_id))).start()
        except Exception as e:
...
def start_command(message):
    ...
    referral_msg = ""
    if referrer_id:
        success, msg_text = process_referral_link(user_id, referrer_id)
        if success:
            referral_msg = f"""
...
║
║ <b>📤 Deploying a bot:</b>
║ • Send a .zip — auto-extracts, installs
║   deps, finds entry point, runs.
║ • Single .py/.js also works.
║
║ <b>🤖 Bot Control:</b>
...
"""
        for i, info in enumerate(running_bots, 1):
            text += f"║ {i}. 🤖 <code>{esc(info['name'][:20])}</code>\n"
            text += f"║    👤 User: {info['user']}\n"
            text += f"║    🆔 PID: {info['pid']}\n"
            text += f"║    ⏱️ Uptime: {info['uptime']}\n"
            text += "║ ──────────────────────────────────\n"
        text += "╚══════════════════════════════════════╝"
    else:
        text = f"""
...
def level_command(message):
    ...
    badge = {'ADMIN': '👑', 'PRO': '⭐', 'FREE': '🆓'}[tier]
    quota_line = f"{used:.0f}/{quota} MB" if quota else "Unlimited"
    next_lvl_pts = (lvl + 1) * 10
    text = f"""
╔══════════════════════════════════════╗
║   🏅 <b>{BRAND_NAME}: YOUR LEVEL</b> 🏅      ║
╠══════════════════════════════════════╣
║
║  {badge} <b>Tier:</b> {tier} · <b>Level:</b> {lvl}
║  ⭐ <b>Points:</b> {points}
║  💾 <b>Storage quota:</b> {quota_line}
║  🔁 <b>Currently running:</b> {running}
║  📦 <b>Hosted bots:</b> {get_current_bot_count(user_id)}/{get_user_max_bots(user_id) if get_user_max_bots(user_id) != float('inf') else '∞'}
║
║  📈 Next level at {next_lvl_pts} points
║     (+{PER_LEVEL_BONUS_BOTS} slots, +{PER_LEVEL_QUOTA_MB} MB quota)
║
╚══════════════════════════════════════╝
"""
    bot.send_message(message.chat.id, text, parse_mode='HTML')
...
def broadcast_command(message):
    ...
    # FIX 7: next_step_handler uses a plain function, not a decorated one
    bot.register_next_step_handler(msg, process_broadcast)


def process_broadcast(message):
    """FIX 7: Called as a next_step handler — safe_command would double-wrap."""
    try:
        ...
        if broadcast_text.startswith('/'):
            bot.reply_to(
                message,
                "⚠️ That looked like a command, not a broadcast message — cancelled. "
                "Run /broadcast again if you actually want to broadcast.")
            return
        progress_msg = bot.send_message(message.chat.id, f"📢 Starting {BRAND_NAME} broadcast...")
        success = 0
        failed = 0
        total = len(active_users)
        # FIX 6: escape broadcast_text since it's raw user input
        escaped_broadcast = esc(broadcast_text)
        for i, user_id in enumerate(list(active_users)):
            try:
                formatted_msg = f"""
╔══════════════════════════════════════╗
...
"""
        ...
        logger.error(f"{BRAND_NAME} process_broadcast crashed: {e}")
        try:
            bot.reply_to(message, f"❌ Broadcast failed: {esc(str(e)[:200])}", parse_mode='HTML')
        except Exception:
            pass
...
def subscribe_command(message):
    ...
    if user_id != OWNER_ID and user_id not in admin_ids:
        bot.reply_to(
            message,
            f"❌ You don't have permission! (Your ID: <code>{user_id}</code>)",
            parse_mode='HTML')
        return
    ...
    if not save_subscription(target_user, expiry):
        bot.reply_to(message, "❌ Subscription could not be saved to the database.")
        return
    with state_lock:
...
"""
    ...
    try:
        bot.send_message(
            ...
            f'Congratulations! You now have <b>PRO Tier</b> access.\n\n'
            f'💾 <b>Storage:</b> 2GB + Bonus\n'
            f'🤖 <b>Slots:</b> 15 + Bonus\n'
            f'⏰ <b>Expires:</b> {expiry.strftime("%Y-%m-%d %H:%M")}\n\n'
            f'Use /level to see your new limits!',
            parse_mode='HTML')
    except Exception:
        pass
...
def unsubscribe_command(message):
    ...
    if not delete_ok:
        bot.reply_to(
            message,
            f"⚠️ Subscription removed from memory but database cancellation failed for "
...
def add_points_command(message):
    ...
    parts = message.text.split()
    if len(parts) < 3:
        bot.reply_to(message, "Usage: /addpoints &lt;user_id&gt; &lt;points&gt;", parse_mode='HTML')
        return
    try:
        target_user = int(parts[1])
        pts = int(parts[2])
    ...
    success = add_points(target_user, pts, f"Admin added {pts} points")
    if success:
...
""", parse_mode='HTML')
    ...
    # FIX 20: also check content-type is an image
    try:
        ...
        if ct and not ct.startswith('image/'):
            bot.reply_to(message, f"❌ URL does not point to an image (Content-Type: {esc(ct)})")
            return
...
def set_description_command(message):
    ...
    if len(parts) < 2:
        bot.reply_to(message, "📖 <b>Usage:</b> /setdesc &lt;description&gt;", parse_mode='HTML')
        return
...
def diagnostics_command(message):
    ...
    channel_line = (f'\n📬 <b>Archive channel:</b> <code>{esc(STORAGE_CHANNEL_ID)}</code>'
                    if _storage_enabled() else '')
    if DB_BACKEND == 'turso':
        storage = 'Turso (connection succeeded ✅)'
    elif DB_BACKEND == 'sqlite-fallback':
        storage = 'Local SQLite fallback (⚠️ Turso unavailable)'
    else:
    ...
    crash_summary = (", ".join(f"{bid}: {len(times)}"
                               for bid, times in bot_crash_counts.items() if times) or "none")

    # FIX 1: show the effective per-file limit
    file_limit = (f"{MAX_SINGLE_FILE_BYTES // (1024 * 1024)} MB (MAX_SINGLE_FILE_MB)"
                  if MAX_SINGLE_FILE_BYTES else "No limit (Telegram download cap applies)")
...
📦 <b>Per-file upload limit:</b> {file_limit}
...
def handle_text(message):
    ...
    if text == "📢 Updates Channel" or norm == "updateschannel":
        ...
        bot.reply_to(message, "❓ Unknown command. Send /help to see all commands.")
    else:
...
"""
    ...
    for i, b in enumerate(bots, 1):
        ...
        text += f"║ {i}. {status} {type_icon} <code>{esc(b['bot_name'][:25])}</code>\n"
...
def handle_document(message):
    ...
    # FIX 13: space check for ALL upload paths, not just zip
    ...
    if MAX_UPLOAD_MB > 0 and file_size > MAX_UPLOAD_MB * 1024 * 1024:
        bot.reply_to(
            message,
            f"❌ File too large! Max {MAX_UPLOAD_MB} MB per file. (This file: {format_size(file_size)})")
        return

    # FIX 1: MAX_SINGLE_FILE_BYTES is now 0 (unlimited) by default
    if MAX_SINGLE_FILE_BYTES > 0 and file_size > MAX_SINGLE_FILE_BYTES:
        bot.reply_to(
            message,
            f"❌ File exceeds the configured limit of {MAX_SINGLE_FILE_BYTES // (1024 * 1024)} MB.")
        return
...
"""
    ...
    # FIX 2: Each upload gets its own fully isolated folder
    ...
    try:
        if file_ext == 'zip':
            ...
            try:
                ...
                if os.path.exists(tmp_zip):
                    os.remove(tmp_zip)
                error_text = ('Invalid or corrupted ZIP!'
                              if isinstance(zip_error, zipfile.BadZipFile)
                              else str(zip_error)[:160])
                bot.edit_message_text(
        ...
        with state_lock:
            user_bots.setdefault(user_id, []).append(bot_entry_dict)

        metadata_saved = save_hosted_bot_db(
            bot_id, user_id, bot_name, folder_name, entry_file, entry_type, file_count)
        if not metadata_saved:
            logger.error(
                f"{BRAND_NAME} Bot {bot_id} metadata was not persisted; "
                "verify the database backend before redeploying.")
...
"""
        ...
        try:
            bot.edit_message_text(
                upload_text + f"║  ❌ Error: {esc(str(e)[:80])}\n╚══════════════════════════════════════╝",
                message.chat.id, progress_msg.message_id, parse_mode='HTML'
            )
        except Exception:
    ...
    # FIX 18: pass the bot_entry_dict directly instead of re-looking it up after a thread race
    threading.Thread(target=_deploy_and_run, args=(bot_entry_dict, message)).start()
...
def _deploy_and_run(bot_entry, message):
    try:
        ...
        if bot_entry['entry_file']:
            ...
            bot.send_message(
                message.chat.id,
                f"📦 <b>{esc(bot_entry['bot_name'])}</b> stored — no runnable .py/.js entry point found, "
        ...
        try:
            bot.send_message(
                message.chat.id,
                f"❌ <b>Deployment hit an unexpected error</b> for "
                f"<code>{esc(bot_entry.get('bot_name', 'bot'))}</code>:\n"
                f"<code>{esc(str(e)[:300])}</code>\n\n"
...
def handle_callback(call):
    ...
    try:
        ...
        logger.error(f"{BRAND_NAME} Callback error for '{data}': {e}")
        try:
...
def share_referral(call):
    ...
    markup.add(
        types.InlineKeyboardButton(
            "📤 Share via Telegram",
            switch_inline_query=f"Join {BRAND_NAME}! {referral_link}"),
...
╠══════════════════════════════════════╣
║
║  {type_icon} <b>Name:</b> <code>{esc(b['bot_name'][:25])}</code>
║  🎯 <b>Entry:</b> <code>{esc((b['entry_file'] or 'None')[:25])}</code>
...
def delete_user_bot_confirm(call, bot_id):
    ...
    if is_bot_running_check(bot_id):
        ...
        return
    confirm_text = f"⚠️ <b>Delete '{esc(b['bot_name'][:25])}'?</b>\n\nThis cannot be undone!"
    markup = types.InlineKeyboardMarkup()
    markup.add(
...
def confirm_delete_bot(call, bot_id):
    ...
    try:
        ...
        logger.error(f"{BRAND_NAME} Delete error for {bot_id}: {e}")
...
def _send_bot_as_file(chat_id, b):
    ...
    try:
        if not ensure_bot_files_available(b):
            bot.send_message(
                chat_id,
                '❌ Bot files are unavailable locally and could not be restored from the storage channel.')
            return
        files_in_folder = []
        for root, dirs, files in os.walk(b['folder']):
            # FIX 8: removed trailing-space typo in condition
        ...
        if len(files_in_folder) == 1:
            ...
            if file_size > TELEGRAM_SEND_SAFE_BYTES:
                bot.send_message(
                    chat_id,
                    f'⚠️ This file is {format_size(file_size)} and exceeds the Telegram-safe '
                    f'send threshold of {TELEGRAM_SEND_SAFE_MB} MB.')
                return
            with open(single_path, 'rb') as f:
                bot.send_document(chat_id, f, caption=f"📄 {esc(b['bot_name'])}")
        else:
            ...
            if archive_size > TELEGRAM_SEND_SAFE_BYTES:
                bot.send_message(
                    chat_id,
                    f'⚠️ The complete ZIP is {format_size(archive_size)} and exceeds the '
                    f'Telegram-safe threshold of {TELEGRAM_SEND_SAFE_MB} MB. '
                    'Open Admin Panel → All User Bots → Download to retrieve individual files.')
                return
            with open(archive_path, 'rb') as f:
                bot.send_document(
                    chat_id, f,
                    caption=f"📦 {esc(b['bot_name'])}.zip ({len(files_in_folder)} files)")
    except Exception as e:
...
def show_all_user_bots_for_admin(call):
    if call.from_user.id != OWNER_ID and call.from_user.id not in admin_ids:
        ...
        return
    if not user_bots:
        bot.edit_message_text("📂 <b>No bots hosted yet!</b>",
    ...
    for uid in sorted(user_bots.keys())[:25]:
        ...
        markup.add(types.InlineKeyboardButton(
            f"{badge} {esc(username[:15])} ({count} bots)",
...
def show_admin_user_bots(call, target_user):
    ...
    for b in bots[:25]:
        ...
        type_icon = "🐍" if b['entry_type'] == 'py' else "🟨" if b['entry_type'] == 'js' else "📦"
        markup.add(types.InlineKeyboardButton(
            f"{status} {type_icon} {b['bot_name'][:20]}",
            callback_data=f"admin_bot_{b['bot_id']}"))
...
👤 Owner: {owner_id}
🤖 Name: <code>{esc(b['bot_name'][:25])}</code>
🎯 Entry: <code>{esc((b['entry_file'] or 'None')[:25])}</code>
📊 Status: {status}
"""
    markup = types.InlineKeyboardMarkup(row_width=2)
...
def _make_zip_browser_token(bot_id, relative_path, action):
    ...
    now = time.time()
    for key in list(zip_browser_tokens.keys()):
        if zip_browser_tokens[key].get('expires', 0) < now:
...
def _resolve_zip_browser(call, token):
    if not _is_admin_user(call.from_user.id):
        bot.answer_callback_query(call.id, '❌ Admin only!')
        return None
    # FIX 9: validate token before using as dict key
    item = zip_browser_tokens.get(token)
    if not item or item.get('expires', 0) < time.time():
        zip_browser_tokens.pop(token, None)
        bot.answer_callback_query(call.id, '⚠️ Menu expired. Open the ZIP browser again.')
        return None
...
def show_admin_zip_browser(call, token_or_bot_id):
    if not _is_admin_user(call.from_user.id):
        bot.answer_callback_query(call.id, '❌ Admin only!')
        return
    # FIX 9: check token dict first, but do NOT use token as a bot_id unless it's not a token
    if token_or_bot_id in zip_browser_tokens:
        ...
        if item.get('expires', 0) < time.time():
            zip_browser_tokens.pop(token_or_bot_id, None)
            bot.answer_callback_query(call.id, '⚠️ Menu expired. Open the file browser again.')
            return
        bot_id = item['bot_id']
        relative = item.get('path', '') or ''
    else:
    ...
    files = []
    try:
        for name in sorted(os.listdir(current), key=lambda v: v.lower()):
    ...
    text = (
        ...
        f"📂 Tap folder to open. 📄 Tap file to download.\n"
    ...
    for name, rel in files[:30]:
        ...
        markup.add(types.InlineKeyboardButton(
            _zip_file_icon(name) + ' ' + name[:28], callback_data='zip_file_' + token))
    if relative:
...
def download_admin_zip_file(call, token):
    ...
    try:
        ...
        if file_size > TELEGRAM_SEND_SAFE_BYTES:
            ...
            bot.send_message(
                call.message.chat.id,
                f'⚠️ <code>{esc(relative)}</code> is {format_size(file_size)} and cannot be sent via Telegram.',
                parse_mode='HTML')
            return
...
def admin_run_bot(call, bot_id):
    ...
    if not ensure_bot_files_available(b, call.message):
        bot.answer_callback_query(call.id, '❌ Files unavailable; restore failed.')
        return
    bot.answer_callback_query(call.id, f"🚀 Running {esc(b['bot_name'])}...")
    threading.Thread(target=run_bot_instance_safe, args=(b, call.message)).start()
...
def admin_stop_bot(call, bot_id):
    ...
    cleanup_script(bot_id)
    time.sleep(1)
    # FIX 12: log admin stop
    log_action(call.from_user.id, "ADMIN_BOT_STOP", f"Admin stopped bot {bot_id}")
...
def admin_delete_bot(call, bot_id):
    ...
    try:
        ...
        remove_hosted_bot_db(bot_id)
        log_action(call.from_user.id, "ADMIN_BOT_DELETE", f"Admin deleted bot {bot_id} (owner {owner_id})")
        bot.answer_callback_query(call.id, "✅ Deleted!")
        bot.send_message(call.message.chat.id, f"✅ Deleted {esc(b['bot_name'])} (owner: {owner_id})")
        show_admin_user_bots(call, owner_id)
    except Exception as e:
        logger.error(f"{BRAND_NAME} Admin delete error for {bot_id}: {e}")
        bot.answer_callback_query(call.id, f"❌ Error: {esc(str(e)[:30])}")
...
def show_top_referrers(call):
    ...
    try:
        ...
        for i, (uid, points, refs) in enumerate(top_users, 1):
            ...
            username = _lookup_username(uid)
            text += f"{medal} {esc(username[:15])} | ⭐{points} | 👥{refs}\n"
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="admin_back"))
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id,
                              parse_mode='HTML', reply_markup=markup)
...
def stop_all_bots(call):
    ...
    for bot_id in bot_ids:
        try:
            ...
            logger.warning(f"{BRAND_NAME} Error stopping bot {bot_id}: {e}")
    # FIX 12: log stop-all
    log_action(call.from_user.id, "ADMIN_STOP_ALL", f"Stopped all bots ({stopped} total)")
...
def cleanup_on_exit():
    ...
    logger.warning(
        f'📢 {BRAND_NAME} Shutdown: stopping bots and attempting best-effort archival.')
    with state_lock:
        # FIX 11: snapshot both collections before any mutations
    ...
    for bot_id, script_info in script_items:
        try:
            kill_process_tree(script_info)
            # Keep was_running=True so startup resume can restore and relaunch
            set_bot_running_state(bot_id, True)
        except Exception as e:
            logger.warning(f"{BRAND_NAME} Shutdown: error stopping {bot_id}: {e}")
            ...
            for b in bots:
                ...
                if os.path.isdir(folder):
                    try:
                        ...
                        logger.warning(f"{BRAND_NAME} Shutdown archive failed for {b['bot_id']}: {e}")
    logger.warning(
        f'{BRAND_NAME} Shutdown finished: {archived} bot(s) archived to Telegram.')
...
def main():
    ...
    logger.info(f"💾 Database backend: {DB_BACKEND}"
                + (f"; last error: {DB_LAST_ERROR}" if DB_LAST_ERROR else ''))
    logger.info(f"📨 Telegram storage: "
                f"{'ON — channel ' + STORAGE_CHANNEL_ID if _storage_enabled() else 'OFF'}")
    logger.info(
        f"📦 Per-file limit: "
        + (f"{MAX_SINGLE_FILE_BYTES // (1024 * 1024)} MB" if MAX_SINGLE_FILE_BYTES else "None (Telegram limit applies)"))
    logger.info("=" * 50)
    ...
    while True:
        try:
            ...
            logger.error(f"{BRAND_NAME} Connection error! Retrying in 10s...")
            time.sleep(10)
        except requests.exceptions.ReadTimeout:
            logger.error(f"{BRAND_NAME} Read timeout! Retrying in 5s...")
            time.sleep(5)
        except Exception as e:
            logger.error(f"{BRAND_NAME} Unexpected error: {e}")
            time.sleep(5)
